...
int shortestPath_BFS(){
	...
	while (!traversingQueue.empty()){
        // Step 3.1: Add the starting node (first node in regionA) to `traversingQueue`.
		Node currentNode = traversingQueue.front();
		Node adjNode; 
		traversingQueue.pop();
		// Step 3.2: process adjNode
		for (int i=0; i<=3; i++){
            adjNode.x = currentNode.x + adjX[i];
            adjNode.y = currentNode.y + adjY[i];
            if (adjNode.x < 0 || adjNode.x >= N) continue; 
            ...
            if (inputArray[adjNode.x][adjNode.y] == 0){
                traversingQueue.push(adjNode);                                                    //Add neighboring vertices to `traversing queue`: those could be node with value `0`                          
                inputArray[adjNode.x][adjNode.y] = inputArray[currentNode.x][currentNode.y]+1;    //Step 3.3: increment the counting
                // counter++;
            }
		}
	}
