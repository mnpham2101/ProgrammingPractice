...
void Solve() {
	...
	// Step 2: gradually decrease the number of smaller box size until remainder == 0
	for (int i = i_max; i >= 0; i--) {              // start from the next smaller box size to the smallest
		biggestBoxSize = BoxSize[i];
		for (int j = sol[i]; j >= 0; j--) {			// decrease the number of smaller box size until zero
			sol[i]--;
			totalBox--;
			totalValue -= BoxSize[i];
			remain = totalValue - N;
			if (remain < biggestBoxSize) {       // if the remainder is larger than the biggest box size => proceed to reduce number of smaller box
				break;
			}
			if (totalValue == N) {
				return;
