...
void runLogic() {
    vector<pair<int, int>> sorted_position_freq;                      // this vector will sort auto in ascending order of frequency
                                                                      // first : position
                                                                      // second: frequency
    vector<int>count(200000, 0);                                      // this vector counts how many time a same frequency is repeatedv
    ...
    count[0] = n - numberOfPositionInSubArray;
    ...
    // Step 3.1 &3.2: calculate number of arrangement, and sum
    for (auto i : sorted_position_freq) {
        int position = i.first;
        int valueOfA_atPosition = A[index];
        index++;
        int freqOfthePosition = i.second;
        sum += valueOfA_atPosition * freqOfthePosition;
        // Step 3.2: count to number of arrangement by the number of repetition for each frequency
    ...
    numberOfArrangement *= factorial(count[0]);
    numberOfArrangement = numberOfArrangement % 100000007;
